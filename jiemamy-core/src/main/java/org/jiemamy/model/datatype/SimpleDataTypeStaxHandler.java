...
public final class SimpleDataTypeStaxHandler extends StaxHandler<SimpleDataType> {
	...
	public SimpleDataType handleDeserialization(DeserializationContext dctx) throws SerializationException {
		...
		try {
			...
			SimpleDataType typeVariant = new SimpleDataType(new SimpleRawTypeDescriptor(category, typeName));
...
//			element.addAttribute(CoreQName.CLASS, model.getClass());
			...
			RawTypeDescriptor rawTypeDescriptor = model.getRawTypeDescriptor();
			element.addElementAndCharacters(CoreQName.TYPE_CATEGORY, rawTypeDescriptor.getCategory());
			element.addElementAndCharacters(CoreQName.TYPE_NAME, rawTypeDescriptor.getTypeName());
