...
/**
 * {@link SimpleJmDeferrabilityStaxHandler}をシリアライズ/デシリアライズするハンドラ。
 * 
 * @version $Id$
 * @author yamkazu
 */
public class SimpleJmDeferrabilityStaxHandler extends StaxHandler<SimpleJmDeferrability> {
	 ...
	 */
	public SimpleJmDeferrabilityStaxHandler(StaxDirector director) {
		super(director);
	}
	
	@Override
	public SimpleJmDeferrability handleDeserialization(DeserializationContext dctx) throws SerializationException {
		...
		try {
			...
			if (StringUtils.isEmpty(text)) {
				...
				return SimpleJmDeferrability.valueOf(text);
			}
	...
	@Override
	public void handleSerialization(SimpleJmDeferrability model, SerializationContext sctx)
