...
/**
 * {@link JmCheckConstraint}をシリアライズ/デシリアライズするハンドラ。
 * 
 * @version $Id$
 * @author yamkazu
 */
public class JmCheckConstraintStaxHandler extends StaxHandler<JmCheckConstraint> {
	
	private static Logger logger = LoggerFactory.getLogger(JmCheckConstraintStaxHandler.class);
	 ...
	 */
	public JmCheckConstraintStaxHandler(StaxDirector director) {
		super(director);
	}
	
	@Override
	public JmCheckConstraint handleDeserialization(DeserializationContext dctx) throws SerializationException {
		...
		try {
			...
			JmCheckConstraint check = new JmCheckConstraint(id);
...
//						check.setDeferrability(SimpleJmDeferrability.valueOf(text));
	...
	@Override
	public void handleSerialization(JmCheckConstraint model, SerializationContext sctx)
