...
/**
 * {@link JmNotNullConstraint}のテストクラス。
 * 
 * @version $Id$
 * @author daisuke
 */
public class JmNotNullConstraintTest {
	
	/**
	 * 適当な {@link JmNotNullConstraint} のインスタンスを作る。
	 * 
	 * @param column 対象カラム
	 * @return {@link JmNotNullConstraint}
	 */
	public static JmNotNullConstraint random(JmColumn column) {
		JmNotNullConstraint nn = new JmNotNullConstraint();
		nn.setName(strNullable());
		nn.setLogicalName(strNullable());
		nn.setDeferrability(enumeNullable(JmDeferrability.class));
