...
/**
 * {@link JmForeignKeyConstraint}のテストクラス。
 * 
 * @version $Id$
 * @author daisuke
 */
public class JmForeignKeyConstraintTest {
	...
	public void test01_find系メソッドのテスト() throws Exception {
		...
		JmTable t1 = new JmTableBuilder("ONE")
				...
				.with(pk1 = JmPrimaryKeyConstraint.of(b))
				.build();
		JmTable t2 = new JmTableBuilder("TWO")
				.with(c = new JmColumnBuilder("C").build())
				.with(d = new JmColumnBuilder("D").build())
				.with(pk2 = JmPrimaryKeyConstraint.of(d))
				.with(fk21 = JmForeignKeyConstraint.of(c, b))
				.build();
