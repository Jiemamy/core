...
public class DiagramTestModelBuilder extends CoreTestModelBuilder {
	...
	private void createPresentations() {
		if (context.hasFacet(DiagramFacet.class)) {
			...
			// ダイアグラム表現の生成・追加(1)
			SimpleJmDiagram presentation1 = new SimpleJmDiagram(uuid.get("1deca0e8-6153-47ad-abe8-ac764f768d96"));
			presentation1.setName("全部表示する");
			
			SimpleDbObjectNode empProfile =
					new SimpleDbObjectNode(uuid.get("aa7caa23-7958-4bd0-a356-8f09d4b74f08"), tableEmp.toReference());
			empProfile.setBoundary(new JmRectangle(360, 60)); // CHECKSTYLE IGNORE THIS LINE
			presentation1.store(empProfile);
			
			SimpleDbObjectNode deptProfile =
					new SimpleDbObjectNode(uuid.get("eb6506ef-4a25-4296-b6d7-c08741f19d5a"), tableDept.toReference());
			deptProfile.setBoundary(new JmRectangle(60, 60)); // CHECKSTYLE IGNORE THIS LINE
			presentation1.store(deptProfile);
			
			SimpleDbObjectNode viewProfile =
					new SimpleDbObjectNode(uuid.get("2387bfd0-7106-44ad-a34e-24231bbea6d5"), viewHighSal.toReference());
			...
			SimpleJmConnection connectionProfile1 =
					new SimpleJmConnection(uuid.get("366b547f-a2a6-42b2-a3a4-38745436c425"), fkEmpEmp.toReference());
			...
			SimpleJmConnection connectionProfile2 =
					new SimpleJmConnection(uuid.get("e5b3d709-2aea-4e26-a90c-b8f438d2da9d"), fkEmpDept.toReference());
			presentation1.store(connectionProfile2);
			
			facet.store(presentation1);
			
			// ダイアグラム表現の生成・追加(2)
			SimpleJmDiagram presentation2 = new SimpleJmDiagram(uuid.get("53c7cdb7-1512-46c7-8ee3-aadec6007896"));
			presentation2.setName("一部表示する");
			
			SimpleDbObjectNode nodeProfile1 =
					new SimpleDbObjectNode(uuid.get("d71ff015-101c-4669-8745-312d1da34efe"), tableEmp.toReference());
			nodeProfile1.setBoundary(new JmRectangle(60, 60)); // CHECKSTYLE IGNORE THIS LINE
			presentation2.store(nodeProfile1);
			
			SimpleDbObjectNode nodeProfile2 =
					new SimpleDbObjectNode(uuid.get("2df7718f-d71e-4dbf-8327-e0ad36f03d5d"), viewHighSal.toReference());
			nodeProfile2.setBoundary(new JmRectangle(270, 270)); // CHECKSTYLE IGNORE THIS LINE
			presentation2.store(nodeProfile2);
			
			SimpleJmConnection connectionProfile =
					new SimpleJmConnection(uuid.get("b615f111-5143-4b12-bfdb-9a06806a9ace"), fkEmpDept.toReference());
