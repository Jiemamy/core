...
/**
 * jiemamy-diagramのリリーステスト。
 * 
 * @version $Id$
 * @author daisuke
 */
@Ignore
public class JiemamyDiagramReleaseTest {
	
	private static Logger logger = LoggerFactory.getLogger(JiemamyDiagramReleaseTest.class);
	...
	private void checkSystemOut(File target, Collection<String> paths) throws IOException {
		if (target.isDirectory()) {
			for (File child : target.listFiles()) {
				...
				&& target.getName().equals("package-info.java") == false
				&& target.getName().equals("JiemamyDiagramReleaseTest.java") == false) {
			String content = FileUtils.readFileToString(target).replaceAll("[\\r\\n]", "");
